...
class CustomQwen2_5_OmniThinkerModel(Qwen2_5OmniThinkerTextModel):
    ...
    def forward(
        ...
        attention_mask: Optional[torch.Tensor] = None,
        feature_attention_mask: Optional[torch.Tensor] = None,
        audio_features_input: Optional[torch.FloatTensor] = None,
        audio_feature_lengths: Optional[torch.LongTensor] = None,
        position_ids: Optional[torch.LongTensor] = None,
        past_key_values: Optional[List[torch.FloatTensor]] = None,
        inputs_embeds: Optional[torch.FloatTensor] = None,
        ...
        # 2. Merge text , audios , image and video
        if input_ids is not None and input_ids.shape[1] != 1:  # Prefill stage
            if audio_features_input is not None:
                # audio_features = self.get_audio_features(
                #     input_features,
                #     feature_attention_mask=feature_attention_mask,
                #     audio_feature_lengths=audio_feature_lengths,
...
class CustomQwen2_5OmniVisionEncoder(Qwen2_5OmniPreTrainedModel):
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        modified from Qwen2_5OmniVisionEncoder.forward
        """
        image_embeds = None
        video_embeds = None
...
class CustomQwen2_5OmniAudioEncoder(Qwen2_5OmniPreTrainedModel):
    _no_split_modules = ["Qwen2_5OmniAudioBlock"]
    ...
    def forward(
        ...
        **kwargs,
        ...
        """
        modified from Qwen2_5OmniAudioEncoder.forward
        """
        if feature_attention_mask is not None:
            audio_feature_lengths = torch.sum(feature_attention_mask, dim=1)
            input_features = input_features.permute(0, 2, 1)[feature_attention_mask.bool()].permute(1, 0)
